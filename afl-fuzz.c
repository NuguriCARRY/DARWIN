...
havoc_stage:
  ...
  u32 variation_intensity = 0;//变异强度
  u32 intensity_best = 0;//最好的变异强度
  u32 use_stacking;//变异强度对应的变异次数
  u32 var_in;//评价时传入，值为当前的变异强度或者最好的变异强度
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    variation_intensity = variation_intensity % 7;

    if(p->tm_min - init_time.minutes > 10 && p->tm_hour + 8 == init_time.hour){ //经历了10分钟以上，最合适的变异强度已经有了
      use_stacking = 1 << (1 + intensity_best);
      var_in = intensity_best;
    }else if(p->tm_hour + 8 > init_time.hour){ //每超过一个小时，重新计算最佳变异强度
      init_time.hour = p->tm_hour + 8;
      init_time.minutes = p->tm_min;
      variation_intensity = 0;
      use_stacking = 1 << (1 + variation_intensity);
      variation_intensity++;
      var_in = variation_intensity;
    }else{//最初10分钟，或者重新开始计算的最初10分钟
      use_stacking = 1 << (1 + variation_intensity);
      variation_intensity++;
      var_in = variation_intensity;
    }
    ...
    u32 mutation_chosen = DARWIN_SelectOperator(darwin_opt_per_seed ? var_in : 0);
    ...
    if (queued_paths != havoc_queued) {
      ...
      intensity_best = DARWIN_NotifyFeedback(darwin_opt_per_seed ? var_in : 0, queued_paths - havoc_queued);
      havoc_queued = queued_paths;
    } else {
      intensity_best = DARWIN_NotifyFeedback(darwin_opt_per_seed ? var_in : 0, 0);
...
int main(int argc, char** argv) {
  ...
  time (&timep);
  p=gmtime(&timep);
  init_time.minutes = p->tm_min; init_time.hour = 8+p->tm_hour;
  ...
  DARWIN_init(7, number_mutations);
